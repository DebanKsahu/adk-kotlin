...
internal open class VertexAiMemoryBankClient(
  ...
  /** Ingests events into a stream for asynchronous, server-side memory generation. */
  open suspend fun ingestEvents(request: IngestEventsRequestDto): Result<Unit> {
    val body = adkJson.encodeToString(IngestEventsRequestDto.serializer(), request)
    return postForUnit(memoriesUrl("memories:ingestEvents"), body, "ingestEvents")
  }
